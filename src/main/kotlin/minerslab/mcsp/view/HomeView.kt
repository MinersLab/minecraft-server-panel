...
class HomeView(
    ...
    private fun createSystemResourceCard(): Card {
        return createCard().apply {
            ...
            val content = Div("0%, 0%")
            add(content, Interval().timeout(2000).once(just = true) {
    ...
    private fun createWelcomeCard() = createCard().apply {
        add(H1("欢迎使用 Minecraft Server Panel"))
    }

    private fun createInstanceCard() = createCard().apply {
    ...
    private fun createRequestCountingCard() = Card().apply {
        setWidthFull()
        title = Div("接口请求")
